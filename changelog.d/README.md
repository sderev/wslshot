...
## Workflow
1. Run `uv run --group dev scriv create`.
...
6. Do not collect fragments manually; release automation will handle `uv run --group dev scriv collect`.
...
- Preview fragments without modifying files: `uv run --group dev scriv print` (returns status 2 if no fragments exist).
