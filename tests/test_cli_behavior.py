...
def test_copy_screenshots_empty_tuple(tmp_path: Path) -> None:
    ...
    result = cli.copy_screenshots((), str(destination))

    assert result == ()


def test_stage_screenshots_multiple_files_one_fails(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Test that staging continues to next file even when one fails."""
    from subprocess import CalledProcessError
    ...
    def selective_fail(cmd, **kwargs):
        ...
        if call_count == 1:
            ...
            staged_files.append(cmd[2])  # cmd is ['git', 'add', '<file>']

    def capture_echo(msg, **kwargs):
        error_msgs.append(str(msg))

    monkeypatch.setattr(cli.subprocess, "run", selective_fail)
    monkeypatch.setattr(cli.click, "echo", capture_echo)

    # Should process both files
    cli.stage_screenshots((Path("shot1.png"), Path("shot2.png")), git_root)

    assert call_count == 2  # Both files attempted
    assert len(staged_files) == 1  # Only second succeeded
    assert staged_files[0] == "shot2.png"
    assert any("Failed to stage" in msg and "shot1.png" in msg for msg in error_msgs)
