...
def test_stage_screenshots_calls_git_add(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Test stage_screenshots() calls git add with correct arguments."""
    git_root = tmp_path / "repo"
    git_root.mkdir()
    ...
    assert called_commands == [
        ["git", "add", "assets/images/shot1.png"],
        ["git", "add", "img/shot2.png"],
    ]


def test_stage_screenshots_runs_from_git_root(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_stage_screenshots_handles_called_process_error(
    ...
    assert "Failed to stage screenshot" in captured.out


def test_stage_screenshots_stages_multiple_files(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Test stage_screenshots() stages multiple files correctly."""
    git_root = tmp_path / "repo"
    git_root.mkdir()
    ...
    # Each file should be staged with a separate git add command
    assert len(called_commands) == 3
    assert called_commands[0] == ["git", "add", "img/shot1.png"]
    assert called_commands[1] == ["git", "add", "img/shot2.png"]
    assert called_commands[2] == ["git", "add", "assets/images/shot3.png"]


def test_stage_screenshots_continues_after_error(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys
) -> None:
    """Test stage_screenshots() continues staging other files after one fails."""
    git_root = tmp_path / "repo"
    git_root.mkdir()
    ...
    def fake_run(cmd, check, cwd):
        """Mock subprocess.run to fail on second call only."""
        call_count[0] += 1
        if call_count[0] == 2:
            raise CalledProcessError(1, cmd)
        return CompletedProcess(cmd, 0)
    ...
    # Should not raise exception even though second file fails
    cli.stage_screenshots(screenshots, git_root)

    # All three files should have been attempted
    assert call_count[0] == 3

    # Should print error for the failed file
    captured = capsys.readouterr()
    assert "Failed to stage screenshot" in captured.out
